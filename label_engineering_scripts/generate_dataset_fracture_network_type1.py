...
TARGET_DATASET_NAME = "Dataset004_fracture_network_type1"
...
def read_img(path: Path) -> tuple[np.ndarray, sitk.Image]:
    """
    Read an image file and return both the array and the SimpleITK object.
...
def write_img(arr: np.ndarray, ref: sitk.Image, path: Path) -> None:
    ...
    arr : np.ndarray
        Output image array.
    ref : sitk.Image
        Reference image that provides spacing, origin, and direction.
    path : Path
        Output path.
    """
...
def compute_adaptive_core_and_boundary(
    inst: np.ndarray,
    group_ids: list[int],
    case_stem: str,
    group_name: str,
    ...
    """
    Compute group mask, adaptive core mask, and boundary mask for one anatomy group.
    ...
    -------
    tuple[np.ndarray, np.ndarray, np.ndarray]
        Group mask, core mask, boundary mask.
    """
    empty = np.zeros_like(inst, dtype=bool)

    if len(group_ids) == 0:
        return empty, empty, empty

    group_mask = np.isin(inst, group_ids)
    core_mask = np.zeros_like(inst, dtype=bool)

    frag_bar = tqdm(group_ids, desc=f"{case_stem} {group_name} core", leave=False)
    ...
    for fid in frag_bar:
        ...
        bbox = get_bbox(frag, pad=BBOX_PAD)
        if bbox is None:
            continue

        frag_crop = frag[bbox]
        dist_crop = distance_transform_edt(frag_crop)
        inside_vals = dist_crop[frag_crop]

        if inside_vals.size == 0:
            continue

        thr = np.percentile(inside_vals, CORE_PERCENTILE)
        thr = max(float(thr), float(MIN_CORE_DIST))

        frag_core_crop = frag_crop & (dist_crop >= thr)

        # Safety fallback for very thin fragments.
        if not np.any(frag_core_crop):
            max_dist = float(dist_crop.max())
            if max_dist > 0:
                frag_core_crop = frag_crop & (dist_crop >= max_dist)

        core_mask[bbox] |= frag_core_crop

    boundary_mask = group_mask & (~core_mask)
    return group_mask, core_mask, boundary_mask
...
def compute_border_mask(
    ...
    """
    Compute the border mask between fragments of the same anatomy group.
    ...
    for fid in frag_bar:
        ...
        bbox = get_bbox(frag, pad=BBOX_PAD + BORDER_RADIUS)
        if bbox is None:
            continue
    ...
    group_mask = np.isin(inst, group_ids)
    group_bbox = get_bbox(group_mask, pad=BORDER_RADIUS)

    if group_bbox is not None:
        group_crop = group_mask[group_bbox]
        near_group_crop = binary_dilation(group_crop, structure=border_structure)
...
def make_group_output(
    ...
    """
    Create the adaptive CBB label output for one anatomy group.
    ...
    group_ids : list[int]
        Fragment IDs belonging to the current group.
    border_structure : np.ndarray
        Structuring element for border dilation.
    case_stem : str
        Case identifier for tqdm labeling.
    group_name : str
        Group name ('LH', 'RH', or 'S').
    ...
    _, core_mask, boundary_mask = compute_adaptive_core_and_boundary(
        inst,
        group_ids,
        case_stem,
        group_name,
    ...
    out_arr[core_mask] = 1
    out_arr[boundary_mask] = 2
    out_arr[border_mask] = 3  # highest priority
...
def create_grouped_labels(
    ...
    """
    Create per-group adaptive CBB labels.
    ...
    desc : str
        Description for tqdm.
    border_structure : np.ndarray
        Structuring element used for border generation.
...
def main() -> None:
    """
    Create Dataset004_fracture_network_type1 from Dataset001_original_data.
