...
TARGET_DATASET_NAME = "Dataset006_fracture_network_type3"
...
def sort_fragments_by_size(inst: np.ndarray, group_ids: list[int]) -> list[tuple[int, int]]:
    """
    Sort fragments of a group by size in descending order.
    ...
    inst : np.ndarray
        Instance label array.
    group_ids : list[int]
        Fragment IDs belonging to one anatomy group.

    Returns
    -------
    list[tuple[int, int]]
        List of (fragment_id, size), sorted by size descending.
    """
    frags = []
    for fid in group_ids:
        size = int(np.count_nonzero(inst == fid))
        if size > 0:
            frags.append((int(fid), size))
...
def compute_border_mask(
    ...
    """
    Compute the border/contact zone between fragments of the same anatomy group.

    A voxel becomes border if dilations of at least two different fragments
    overlap there.
...
def make_group_output(
    ...
    """
    Create the type-3 label output for one anatomy group.
    ...
    1 = biggest fragment
    2 = smaller fragments
    3 = border/contact zone between fragments of same group

    Border has highest priority and overwrites 1/2.
    ...
    frags = sort_fragments_by_size(inst, group_ids)
    if len(frags) == 0:
        return out

    biggest_fid = frags[0][0]
    out[inst == biggest_fid] = 1
    ...
    border_mask = compute_border_mask(
        inst=inst,
        group_ids=[fid for fid, _ in frags],
...
def create_grouped_labels(
    ...
    """
    Create type-3 bone-specific labels.
...
def main() -> None:
    """
    Create Dataset006_fracture_network_type3 from Dataset001_original_data.
