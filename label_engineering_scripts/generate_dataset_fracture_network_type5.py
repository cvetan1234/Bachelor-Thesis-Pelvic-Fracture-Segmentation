...
TARGET_DATASET_NAME = "Dataset008_fracture_network_type5"
...
def compute_border_mask(
    ...
    """
    Compute overlap/contact zones between fragments of the same group.

    The same overlap logic as the border-label variant is used, but the result
    will later be removed from the bone instead of being written as a separate class.
    ...
    border_structure : np.ndarray
        Structuring element for initial overlap detection.
    thick_structure : np.ndarray
        Structuring element for extra thickening of the removed zone.
    case_stem : str
    ...
    -------
    np.ndarray
        Boolean overlap/removal mask.
    """
    if len(group_ids) < 2:
        return np.zeros_like(inst, dtype=bool)

    dilated_sum = np.zeros_like(inst, dtype=np.uint16)
    frag_bar = tqdm(group_ids, desc=f"{case_stem} {group_name} overlap", leave=False)
...
def make_group_output(
    ...
    """
    Create the type-5 label output for one anatomy group.

    Output labels
    -------------
    0 = background / removed separation
    1 = bone after removing overlap/contact regions
    ...
    border_structure : np.ndarray
        Structuring element for initial overlap detection.
    thick_structure : np.ndarray
        Structuring element for thickening the removed zone.
    case_stem : str
        Case identifier for tqdm labeling.
    group_name : str
        Group name ('LH', 'RH', or 'S').
    ...
    final_bone = group_mask & (~border_mask)
    out_arr[final_bone] = 1
...
def create_grouped_labels(
    ...
    """
    Create type-5 bone-specific labels.
    ...
    desc : str
        Description for tqdm.
    border_structure : np.ndarray
        Structuring element for overlap detection.
    thick_structure : np.ndarray
        Structuring element for thickening the removed zone.
...
def main() -> None:
    """
    Create Dataset008_fracture_network_type5 from Dataset001_original_data.
