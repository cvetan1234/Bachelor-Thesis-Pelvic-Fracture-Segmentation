...
DEFAULT_RESTORATION_SUBDIR = "restore_type5_script1"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional output folder. Default: restorations/restore_type5_script1_auto",
    )

    parser.add_argument("--min_seed_voxels", type=int, default=20)
...
def split_class_with_prediction(class_mask, pred_arr, min_seed_voxels):
    if not np.any(class_mask):
        return np.zeros_like(class_mask, dtype=np.int32)

    seed_mask = (pred_arr == 1) & class_mask
    ...
    missing_mask = class_mask & (cc == 0)
    full = assign_to_nearest(cc, missing_mask)
...
def main():
    ...
    fracture_cases = collect_fracture_cases(fracture_dir)  # 🔁 renamed
    ...
    for anatomy_path in tqdm(anatomy_files, desc="Cases"):
        ...
        _, pred_LH = read_image(fracture_cases[cid]["LH"])
        _, pred_RH = read_image(fracture_cases[cid]["RH"])
        _, pred_S = read_image(fracture_cases[cid]["S"])
        ...
        for cls, pred_arr, offset in class_info:
